...
class PrincessBot {
  ...
  setupWebSocketHandlers() {
    this.websocketManager.onAction((data) => {
      this.handleActionReceived(data);
    });

    this.websocketManager.onStatus((status) => {
  ...
  connectToBackend() {
    ...
    this.websocketManager.connect("http://localhost:3001");
  ...
  async sendTextInput() {
    ...
    try {
      ...
      if (this.websocketManager.isConnected) {
        this.websocketManager.sendTextInput(text);
